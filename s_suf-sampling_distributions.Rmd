---
title: 'Data 606 Lab5a - Foundations for statistical inference - Sampling distributions'
author: 'Sufian'
output:
  html_document:
    css: ./lab.css
...
The data is mostly normal with the bulk between 1000 to 2000 but there's skewness especially to the 

right.  These long tails are evidenced below using the Q-Q plot where one can see the deviations off the
...
The sample of 50 homes is still nearly normal, see histogram below.  It looks similar to its parent

dataset, having a tail of higher than average home sizes.
...
From the above data frame of means of all samples, notice that the sample of 1000 is closer to the

population mean. 

This tells us that as sample size gets larger, the closer we get to population mean. 

Sample size of 1000 is more accurate estimate of the population mean.
...
dfMeans = data.frame(mean(samp1), mean(samp2), mean(samp3), mean(samp4))
print(dfMeans)
```
...
sample_means50 has 5000 elements, see below. The sample distribution looks unimodal with normal shape,

normal plot above.

From the Q-QPlot, the distribution seems normal as it mostly adhere to the 45 deg line. If we collected

50,000 samples, then the distribution would be more of the normal distribution. 
...
The center gets closer to the population mean while the spread gets smaller.
...
samplemean_price_50 = mean(price)
...
The distribution looks normal, sample mean is about 180,000 which is close to the population mean of
...
hist(sample_means150, breaks = 30, xlim = xlimits)

samplemean_price_150 = mean(price)

samplemean_price_150
```
...
The new sample distribution is still very normal looking.  

My answer is still similar but my new mean based on this sampling size is slgihtly higher;  I would guess

it to be around 180,150 in that area.  
...
The smaller spread comes with the larger sample size. Therefore, the sample distribution with sample

size 150 has a smaller spread. 

For better accuracy in pricing, use the smaller spread model

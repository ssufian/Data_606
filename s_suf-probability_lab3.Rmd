...
title: Data 606 Lab3 "Probability"
...
Streak 1 means One hit followed by a miss

Streak 0 means NO hits and one miss
...
outcomes <- c("H", "M")
sim_basket1 <- sample(outcomes, size = 1, replace = TRUE)
...
add the prob = C(0.45,0.55) argument, see below

```{r}

sim_basket1 <- sample(outcomes, size = 133, replace = T, prob = c(.45, .55))
...
The Kobe actual results compared to the simulated results (with independenc) is very similar.  With this simulated results, we showed that Kobe does not have a "Hot Hand".
...
```
```{r}
#barplot of shots taken

barplot(table(sim_streak1))
```
...
(b) typically his streak is of lenght 0

(c) Max streak is zero streak (0)

(d) Mean of 0.595 with Std Dev. of 0.98
...
The distribution should be similar.  Given the same conditions of :

- low probability of single hits

- long zero streaks

- Independence

Moreover, the mulitplication rule will actually cofirms why "hot hands" do not exist; because with each

multiplication of the probability, the multiplied probability reduces.  Therefore, higher lenght streaks 

becomes less likely which aligns with Kobe's actual results of hits vs. misses.
...
```{r}
# lets look at Kobe's actual shooting results

barplot(table(kobe_streak))
```
```{r}
# compare that to our simulated results
...
The two distributions (Kobe's vs. our simulated) are almost identical. 

Summary of our results (compare and contrast):

- Kobe's mean is higher

- Std Dev is also lower, is very similar

- same long zero streaks with very low high streaks

These 2 similar results yet again showed there's no such thing as "Hot Hands"
